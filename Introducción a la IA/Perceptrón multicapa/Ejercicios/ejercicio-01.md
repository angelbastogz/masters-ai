...
1. Enlaces de Colab (o archivos `.ipynb`) de las dos notebooks **modificadas**,
   ...
   * [04 Multilayer perceptron.ipynb](../Notebooks/04%20Multilayer%20perceptron.ipynb)
   * [05 Keras - multilayer perceptron - iris.ipynb](../Notebooks/05%20Keras%20-%20multilayer%20perceptron%20-%20iris.ipynb)
...
##### 2 capas
  ![mlp - 2 layers](../evidencia/mlp_2_layers.png)

##### 4 capas
  ![mlp - 4 layers](../evidencia/mlp_4_layers.png)

#### Keras
##### 2 capas
  ![keras - 2 layers - summary](../evidencia/keras_2_layers_summary.png)

  ![keras - 2 layers](../evidencia/keras_2_layers.png)

##### 4 capas
  ![keras - 2 layers - summary](../evidencia/keras_4_layers_summary.png)

  ![keras - 2 layers](../evidencia/keras_4_layers.png)


3. Un breve reporte (media página a una página) que responda:
   - ¿Baja más el error al añadir dos capas, o se estancó / empeoró? ¿Igual
     en NumPy y en Keras?
   > Al añadir más capas, se puede notar que hay un estancamiento inicialmente y en el NumPy se notá que después de 300 iteraciones
   > aparece una disminución pronunciada del error. En Keras, por otro lado, se nota un estancamiento del error y no se compone
   ...
   > No se parecen, en el numpy se nota que hay una disminución pronunciada del error al principio, luego hay un ligero estancamiento y luego vuelve a disminuir.
   > Por otro lado, en keras, se aprecia una disminución constante del error. 
   > Se debe primero por el orden de los datos, el numpy recorre las flores siempre en el mismo orden, mientras que Keras
   > resuelve los datos en cada época, entonces aprende de forma más pareja. De igual forma, en el numpy los pesos se calculan de forma aleatoria,
   > y Keras utiliza su propio método de inicialización que da números en un rango diferente. 
   > Otro factor es que el numpy mide el error sumando el error de las 3 salidas y Keras utiliza el promedio de esas 3 salidas. 
   ...
   > Si, tiene sentido. Se debe sobre todo que Iris es un problema sencillo, con pocas muestras, caracteristicas y clases. 
   > Al añadir más capas, la neurona se satura al acercarse más a 0 lo que ocasiona el estancamiento. 
   > En esté problema en particular, añadir más capas es innecesario y afecta el resultado. 
