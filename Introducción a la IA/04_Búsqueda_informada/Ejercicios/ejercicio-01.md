...
![Subgrafo](../evidencia/subgrafo_oradea_bucharest.png)

2. Una tabla comparativa con Path, Depth, Cost, Expanded (y la heurística
   usada).

| Algoritmo         | Path                                                      | Depth   | Cost (km) | Expanded | Generated | Heurística                                       |
|-------------------|-----------------------------------------------------------|---------|-----------|----------|-----------|--------------------------------------------------|
| Greedy best-first | Oradea -> Sibiu -> Fagaras -> Bucharest                   | 3 roads | 461       | 3 nodes  | 9 nodes   | Straight-line distance to Bucharest (AIMA table) |
| A*                | Oradea -> Sibiu -> Rimnicu Vilcea -> Pitesti -> Bucharest | 4 roads | 429       | 5 nodes  | 15 nodes  | Straight-line distance to Bucharest (AIMA table) |

3. Un breve reporte (media página) que responda:
   - ¿A* encontró el camino de **menos km**? ¿Greedy coincidió o se desvió?
   > Si, A* encontró el camino de menos km. Greedy, por otro lado, se desvió. 
   
   - ¿Por qué Greedy puede devolver un camino más caro aunque `h` sea
     admisible?
   >  Porque Greedy únicamente considera la heurística n en su función de evaluación. Ignora el costo ya recorrido, por lo que
   > puede devolver un camino más caro como vemos en esté caso. 
   ...
   > Si, f nunca disminuye a lo largo del camino, y esto se debe a que h es consistente. La consistencia establece que la heurística en un nodo
   > nunca puede ser mayor que el costo real de dar un paso hacia el siguiente nodo más la heurística de ese nodo. 
...
#### 03_greedy_best_first_search
  ![03_greedy_best_first_search](../evidencia/03_greedy_best_first_search_oradea_bucharest.png)

#### 04_a_star_search
  ![04_a_star_search](../evidencia/04_a_star_search_oradea_bucharest.png)
