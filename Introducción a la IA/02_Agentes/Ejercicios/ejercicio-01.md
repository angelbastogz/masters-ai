...
1. El archivo [`config/mi_cueva_4x4.yaml`](../config/mi_cueva_4x4.yaml).
...
>Solamente un agente logro salir con el oro en el mapa que he creado, ese agente fue el Utility based agent.
>Los otros agentes no lo lograron, noté que el model based agent y el goal based agent se atoraron en la misma coordenada.
>Mientras que el simple reflex agent al percibir un Breeze ya no continue moviendose de coordenada.
   - ¿Por qué el **agente de reflejo simple** falla (o tiene suerte) en tu diseño?
>Es un agente muy básico. Solamente ejecuta una acción basandose en la percepción actual y no mantiene una memoria de acciones
>Por lo que es muy fácil que termine en un bucle. 
>Revisando el código del agente se puede notar que es un codigo sencillo que ejecuta acciones utilizando simplemente la sentencia `if`
>Por lo que al percibir breeze o stench ejecutará siempre la misma acción que es `TURN_RIGHT` lo que ocasiona que se cree un ciclo infinito.
   - ¿Cómo cambia el resultado del **agente basado en modelo** si acercas o alejas
     un pit de la casilla inicial?
>Al acercar un pit de la casilla inicial ocasiona que el agente no se mueva, permanece en la posición 1,1. Al alejar el pit
>el agente logra moverse de posición aunque aún existe la posibilidad de que se genere otro bucle infinito en donde el agente se quede 
>estancado en su posición.

4. Evidencias (captura de pantalla) de haber corrido los 4 agentes con tus nueva configuración del mundo.

### Agente de reflejo simple (`02_simple_reflex_agent.py`)                                                                                                                                                                       
  ![Reflejo simple](../evidencia/02_simple_reflex_agent.png)

### Agente basado en modelo (`03_model_based_agent.py`)                                                                                                                                                                          
  ![Basado en modelo](../evidencia/03_model_based_agent.png)     

### Agente basado en metas (`04_goal_based_agent.py`)                                                                                                                                                                            
  ![Basado en metas](../evidencia/04_goal_based_agent.png)   

### Agente basado en utilidad (`05_utility_based_agent.py`)                                                                                                                                                                      
  ![Basado en utilidad](../evidencia/05_utility_based_agent.png)     

### Agente de aprendizaje (`06_learning_agent.py`)                                                                                                                                                                               
  ![Agente de aprendizaje](../evidencia/06_learning_agent.png)   
...
### Agente basado en modelo (`03_model_based_agent.py`)                                                                                                                                                                          
  ![Basado en modelo](../evidencia/03_model_based_agent_mi_cueva_dificil.png)     

### Agente basado en metas (`04_goal_based_agent.py`)                                                                                                                                                                            
  ![Basado en metas](../evidencia/04_goal_based_agent_mi_cueva_dificil.png)

> No logre encontrar un caso donde el goal based agent dispare. 
