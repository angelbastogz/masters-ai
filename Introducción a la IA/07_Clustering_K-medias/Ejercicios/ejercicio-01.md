...
1. Enlace de Colab (o el `.ipynb` modificado) con la corrida original y
   ...
   > [K-medias.ipynb](../Notebooks/01%20K-medias.ipynb)

2. Capturas: scatter, Voronoi \(k = 5\), codo y silueta — **original** y
   **modificado** (8 figuras, o 4 pares).

   ### Scatter original
   ![Blobs plot Scatter](../evidencia/blobs_plot_scatter.png)

   ### Voronoi original
   ![Voronoi K = 5](../evidencia/voronoi_k_5.png)
   
   ### Codo original
   ![Codo original](../evidencia/inercia_codo.png)

   ### Silueta original
   ![Silueta original](../evidencia/silhouette_score_vs_k_plot.png)


   ### Scatter modificado
   ![Blobs plot Scatter modificado](../evidencia/blobs_plot_scatter_modified.png)

   ### Voronio modificado
   ![Voronoi modified](../evidencia/voronoi_modified.png)

   ### Codo modificado
   ![Codo modificado](../evidencia/inercia_codo_modified.png)

   ### Silueta modificado
   ![Silueta modificado](../evidencia/silhouette_score_vs_k_plot_modified.png)
...
4. Un breve reporte (media página) que responda:
   ...
   > Porque dos de los cinco centros de géron están casi pegados entre sí en comparación con la separación del resto, y como
   > el método del codo detecta el número de clusters a partir del cual seguir agregando ya no reduce mucho la varianza, al llegar a
   > 4 clusters y pasar a 5 esa reducción es menor por lo que ese método prefiere k = 4.

   - Con tus blobs separados, ¿el codo y la silueta coinciden en el mismo
     \(k\)? ¿Ese \(k\) es 5?
   > Si, ambos me indicaron K = 5. 

   - Si el codo sigue en 4, ¿qué te falta mover (distancia entre centros
     vs. `blob_std`)?
   > En mi caso, el codo si me dió 5 como resultado. Lo que hice fue separar los centros y reducir la disperción de los puntos
   > para que el algoritmo pudiera identificar mejor los clusters. 

5. Evidencia de haber ejecutado en Colab (captura del entorno o del menú
   Runtime).
   ![colab](../evidencia/colab.png)
