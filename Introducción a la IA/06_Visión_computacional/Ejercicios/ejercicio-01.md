...
1. Enlace de Colab (o el `.ipynb` modificado) con la corrida original y la
   ...
   > [13 YOLO ultralytics](../Notebooks/13%20YOLO%20ultralytics.ipynb)

2. Capturas: salida sobre `zidane.jpg`, sobre `bus.jpg` y sobre **tu**
   foto.

   ### zidane.jpg
   ![zidane.jpg](../evidencia/zidane.jpg)

   ### bus.jpg
   ![bus.jpg](../evidencia/bus.jpg)

   ### mifoto.
   ![mifoto.jpg](../evidencia/mifoto.jpg)

3. Un breve reporte (media página) que responda:
   - ¿Qué clases detectó YOLO en las fotos de Ultralytics y cuáles en la
     tuya?
   > En las fotos de Ultralytics detecto person, tie, bus, y stop sign. 
   > En mi foto detecto person, bicycle y handbag.

   - ¿Algún objeto evidente de tu foto **no** salió etiquetado? ¿Por qué
     podría pasar (clase que no está en COCO, objeto chico, recorte,
     umbral de confianza)?
   > No detecto umbrella, tal vez porque está ubicado en el fondo de la imagen. 

   - ¿La predicción de la celda CLI y la de `model(...)` coinciden sobre
     tu misma imagen?
   > Si, detecta las mismas clases. Lo que varía ligeramente es el nivel de confianza de cada clase detectada.
   > En la mayoría de las clases detectadas, la confianza es mayor en el CLI aunque no en todos los casos fue así. 

4. Evidencia de haber ejecutado en Colab (captura del entorno o del menú
   Runtime, idealmente con GPU).

   ![colab-entorno.png](../evidencia/colab-entorno.png)
